...
public class FlowLikeView extends RelativeLayout {
    ...
    private Handler mHandler = new Handler(){
        ...
        public void handleMessage(Message msg) {
            if (msg.what == 0 && isStart) {
                addLikeView();
                mHandler.sendEmptyMessageDelayed(0,mRandom.nextInt(1000)+200);
            }
        }
    };
    ...
    public void initParams(List<Integer> resId) {
        mLikeDrawables = new ArrayList<>();

        for (int res :resId) {
            mLikeDrawables.add(generateDrawable(res));
        }
...
//        mHandler.sendEmptyMessageDelayed(0,1000);
    ...
    private ValueAnimator generateCurveAnimation(View target) {
        ...
        ValueAnimator valueAnimator = ValueAnimator.ofObject(evaluator,
                new PointF((mViewWidth - mPicWidth) / 2, mViewHeight - mPicHeight ),
    ...
    private PointF generateCTRLPointF(int value) {
        ...
        try {
            ...
            pointF.y = mRandom.nextInt(mViewHeight / value)+1;
        }catch (Exception e){
            pointF.x = 0;
            pointF.y = 0;
        }
